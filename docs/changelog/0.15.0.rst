...
.. code-block:: python
    :caption: For Lakekeeper, Polaris, Gravitino
...
- Allow to set ``SparkS3(path_style_access=True)`` instead  of ``SparkS3(extra={"path.style.access": True)`` (:github:pull:`392`).
  ...
  - Add a runtime warning about missing ``S3(region=...)`` and ``SparkS3(region=...)`` params (:github:pull:`418`).

  It is recommended to explicitly pass this parameter to avoid potential access errors.

Thanks to :github:user:`yabel`
